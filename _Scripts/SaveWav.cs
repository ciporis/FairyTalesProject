...
public class SaveWav
{
    ...
    {
        string file = "";
        return FromAudioClip(audioClip, file, false);
    }
    ...
    public static byte[] FromAudioClip(AudioClip audioClip, string filepath, bool saveAsFile = true)
    {
        ...
        if (saveAsFile)
        {
            //filepath = string.Format("{0}/{1}/{2}.{3}", Application.dataPath + $"/Resources/Sounds/FairyTalesAudio/{BookData.FairyTaleName}", dirname, DateTime.UtcNow.ToString("yyMMdd-HHmmss-fff"), "wav");
